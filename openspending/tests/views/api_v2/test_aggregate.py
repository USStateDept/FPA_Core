...
class TestSearchApiController(ControllerTestCase):
    ...
    #     response = self.client.get(url_for('api_v2.aggregate', dataset='sci_study'))
    ...
    #                        (u'pages', 1),
    #                        (u'pagesize', 10000)]
    #     assert sorted(result['summary'].items()) == expected_result, sorted(result['summary'].items())

    # def test_aggregate_drilldown(self):
    #     response = self.client.get(url_for('api_v2.aggregate',
    #                                        dataset='sci_study',
    #                                        drilldown='country_level0'))
    #     assert '200' in response.status

    #     result = json.loads(response.data)
    #     assert result['summary']['num_drilldowns'] == 6, result['summary']['num_drilldowns']
    #     assert result['summary']['amount'] == -371500000.0, result['summary']['amount']
    ...
    #     response = self.client.get(url_for('api_v2.aggregate',
    #                                        dataset='sci_study', cut='year:2009',
    #                                        measure='total'))
